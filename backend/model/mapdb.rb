class MAPDB
  ...
  class DBPool
    ...
    def initialize(pool_size = AppConfig[:map_db_max_connections], opts = {})
      @pool_size = pool_size
      @opts = opts
    end
    ...
    def connect
      if not @pool
        ...
        begin
          Log.info("Connecting to database: #{AppConfig[:map_db_url_redacted]}. Max connections: #{pool_size}")
          pool = Sequel.connect(AppConfig[:map_db_url],
                                :max_connections => pool_size,
                                :test => true,
                                :loggers => (AppConfig[:map_db_debug_log] ? [Logger.new($stderr)] : [])
                               )
  ...
  MAPDB._generate_instance_methods!
end
